...
def test_apply_long_buy_basic(portfolio: Portfolio) -> None:
    executed = portfolio.apply_long_buy("TTWO", quantity=100, price=50.0)
    assert executed == 100
    snap = portfolio.get_snapshot()
    assert snap["positions"]["TTWO"]["long"] == 100
    assert snap["positions"]["TTWO"]["long_cost_basis"] == pytest.approx(50.0)
...
def test_apply_long_buy_partial_fill_when_insufficient_cash() -> None:
    p = Portfolio(tickers=["TTWO"], initial_cash=120.0, margin_requirement=0.5)
    # Request 10 shares at $20 = $200, but only $120 cash → max 6 shares
    executed = p.apply_long_buy("TTWO", quantity=10, price=20.0)
    assert executed == 6
    snap = p.get_snapshot()
    assert snap["positions"]["TTWO"]["long"] == 6
...
def test_apply_long_sell_realized_gain_and_cost_basis_reset(portfolio: Portfolio) -> None:
    ...
    portfolio.apply_long_buy("TTWO", 100, 50.0)
    executed = portfolio.apply_long_sell("TTWO", 100, 60.0)
    assert executed == 100
    snap = portfolio.get_snapshot()
    assert snap["positions"]["TTWO"]["long"] == 0
    assert snap["positions"]["TTWO"]["long_cost_basis"] == pytest.approx(0.0)
    assert snap["realized_gains"]["TTWO"]["long"] == pytest.approx(1_000.0)
    # Cash: initial 100k - 5k + 6k = 101k
    assert snap["cash"] == pytest.approx(101_000.0)


def test_apply_long_sell_clamps_to_owned() -> None:
    p = Portfolio(tickers=["TTWO"], initial_cash=10_000.0, margin_requirement=0.5)
    p.apply_long_buy("TTWO", 10, 100.0)
    # Try to sell 20, but only 10 owned
    executed = p.apply_long_sell("TTWO", 20, 100.0)
    assert executed == 10
    assert p.get_snapshot()["positions"]["TTWO"]["long"] == 0


def test_apply_short_open_basic(portfolio: Portfolio) -> None:
    # Short 100 @ $30, margin 50% → proceeds 3,000; margin 1,500
    executed = portfolio.apply_short_open("LUG", 100, 30.0)
    assert executed == 100
    snap = portfolio.get_snapshot()
    pos = snap["positions"]["LUG"]
...
def test_apply_short_open_partial_when_insufficient_margin_cash() -> None:
    ...
    p = Portfolio(tickers=["TTWO"], initial_cash=200.0, margin_requirement=0.5)
    # price=100 → margin per share = 50, cash 200 → max 4 shares
    executed = p.apply_short_open("TTWO", 10, 100.0)
    assert executed == 4
    snap = p.get_snapshot()
    pos = snap["positions"]["TTWO"]
    assert pos["short"] == 4
...
def test_apply_short_cover_realized_gain_and_margin_release(portfolio: Portfolio) -> None:
    ...
    portfolio.apply_short_open("TTWO", 100, 50.0)
    pre = portfolio.get_snapshot()
    pre_margin_used = pre["positions"]["TTWO"]["short_margin_used"]
    executed = portfolio.apply_short_cover("TTWO", 40, 40.0)
    assert executed == 40
    snap = portfolio.get_snapshot()
    pos = snap["positions"]["TTWO"]
    assert snap["realized_gains"]["TTWO"]["short"] == pytest.approx(400.0)
...
def test_apply_short_cover_clamps_to_existing_short() -> None:
    p = Portfolio(tickers=["TTWO"], initial_cash=10_000.0, margin_requirement=0.5)
    p.apply_short_open("TTWO", 5, 100.0)
    executed = p.apply_short_cover("TTWO", 10, 100.0)
    assert executed == 5
    assert p.get_snapshot()["positions"]["TTWO"]["short"] == 0
...
def test_zero_or_negative_quantity_is_noop(portfolio: Portfolio, action: str) -> None:
    ...
    if action == "buy":
        executed = portfolio.apply_long_buy("TTWO", 0, 10.0)
        executed2 = portfolio.apply_long_buy("TTWO", -5, 10.0)
    elif action == "sell":
        executed = portfolio.apply_long_sell("TTWO", 0, 10.0)
        executed2 = portfolio.apply_long_sell("TTWO", -5, 10.0)
    elif action == "short":
        executed = portfolio.apply_short_open("TTWO", 0, 10.0)
        executed2 = portfolio.apply_short_open("TTWO", -5, 10.0)
    else:
        executed = portfolio.apply_short_cover("TTWO", 0, 10.0)
        executed2 = portfolio.apply_short_cover("TTWO", -5, 10.0)
